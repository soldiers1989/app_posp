...
public class YPLThread extends Thread{
	...
	public YPLThread(ITotalPayService service, DaifuRequestEntity payRequest, PmsMerchantInfo merchantinfo) {
		super();
	...
	public void run() {
		...
		try {
			...
			for (int i = 0; i < 30; i++) {
			log.info("进入循环了");
			 map= service.yplQuick(payRequest.getV_mid(), payRequest.getV_batch_no());
			 log.info("查询完了");
			 if(map!=null){
