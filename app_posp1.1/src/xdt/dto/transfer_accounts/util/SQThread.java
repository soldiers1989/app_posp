...
public class SQThread extends Thread{
	...
	public SQThread(ITotalPayService service, DaifuRequestEntity payRequest, PmsMerchantInfo merchantinfo) {
		super();
	...
	public void run() {
		...
		try {
			...
			for (int i = 0; i < 20; i++) {
			log.info("进入循环了");
			 map= service.sqQuick(payRequest.getV_mid(), payRequest.getV_batch_no());
			 log.info("查询完了");
			 if(map!=null){
						 ...
						 service.UpdateDaifu(payRequest.getV_batch_no(), "02");
							...
							if("0".equals(payRequest.getV_type())) {
				 			...
				 			if(nus==1) {
								...
								log.info("双乾补款订单状态："+ii);
				 			} 
				 		 break;
					 }else if("1003".equals(map.get("v_status"))){
						 service.UpdateDaifu(payRequest.getV_batch_no(), "03");
						 break;
					 }
				 }
